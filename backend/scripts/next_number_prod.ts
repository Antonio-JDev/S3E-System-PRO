...
async function main(){
  ...
  const raw = await prisma.<{ max: number }[]>(
    'SELECT MAX(CAST(numero AS INTEGER)) as max FROM notas_fiscais WHERE empresaFiscalId = docker compose exec -T backend sh -lc "cat > /app/backend/scripts/next_number_prod.ts <<'TS'
import { PrismaClient } from '@prisma/client';
...
main();
TS" AND ambiente =  AND numero ~ ''^[0-9]+$''',
    empresa.id,
    'PRODUCAO'
  );
  const maxExisting = (raw && raw[0] && raw[0].max) ? Number(raw[0].max) : 0;
